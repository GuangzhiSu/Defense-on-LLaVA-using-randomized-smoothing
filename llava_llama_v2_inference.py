...
def add_noise_to_text(text, frequency=0.1, perturbation_type='character', severity=0.1):
    """
    Add noise to text data by introducing perturbations.
    
    :param text: Input text (string) to be perturbed
    :param frequency: Proportion of characters/words to modify (between 0 and 1)
    :param perturbation_type: Type of perturbation ('character', 'word')
    :param severity: Degree of perturbation. For character noise, it controls how severe the swaps/inserts are. 
                     For word-level noise, it controls how likely a word will be replaced with its synonym.
    :return: Noisy version of the text
    """
    
    def add_character_noise(text, swap_prob):
        chars = list(text)
    ...
    def add_synonym_noise(text, replace_prob):
        words = text.split()
        noisy_words = [replace_with_synonym(word) if random.random() < replace_prob else word for word in words]
        return ' '.join(noisy_words)
    
    # Apply different types of noise based on the chosen perturbation type
    if perturbation_type == 'character':
        # Adjust severity by increasing or decreasing swap probability
        noisy_text = add_character_noise(text, swap_prob=severity)
    elif perturbation_type == 'word':
        # Frequency controls the number of words perturbed, severity controls the likelihood of replacing with a synonym
        noisy_text = add_synonym_noise(text, replace_prob=frequency)
    else:
        raise ValueError("Invalid perturbation type. Choose either 'character' or 'word'.")
...
new_images = _apply_random_smoothing(image, sigma = 0.29, num_copy = 10)
...
with torch.no_grad():
    ...
    for i, user_message in enumerate(datasets):
        ...
        repeated_text_prompts = [
                add_noise_to_text(text_prompt % user_message, frequency=0.1, perturbation_type='word', severity=0.1)
                for _ in range(num_copy)
            ]
